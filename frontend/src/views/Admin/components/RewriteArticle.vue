...
import { getAdminArticle, updateArticle } from "@/utils/article"
...
onMounted(async () => {
  ...
  const res = await getAdminArticle(slug)
  const data = res.data.data
  form.tags = data.tags

  form.title = data.title
  form.mdFile = data.content
  form.isDraft = data.is_draft

  if (data.cover) {
  coverFile.value = [
    {
      name: data.cover.split("/").pop(),
      url: data.cover
    }
  ]
}
...
const submitForm = async () => {
    ...
    if (coverFile.value.length > 0 && coverFile.value[0].raw) {
    ...
    try {
    await updateArticle(route.params.slug, formData)
    ElMessage.success(form.isDraft ? "草稿保存成功！" : "修改成功，已发布！")
  } catch (err) {
    const msg =
      err.response?.data?.msg ||
      err.response?.data?.error ||
      (form.isDraft ? "保存失败！" : "修改失败！")
