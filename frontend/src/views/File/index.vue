...
const validTags = computed(() => {
  return tagsStore.tags.filter(tag => tag.article_count > 0)
})
...
const filteredArticles = computed(() => {
  ...
  return articles.value.filter(article =>
    article.tags.some(tag => tag.id === activeTag.value)
  )
})
...
onMounted(async () => {
  loading.value = false
  await articleStore.fetchArticles(page, 100);
})
        ...
        <transition-group name="fade-slide" tag="div">
          <RouterLink v-for="post in group.posts" :key="post.slug" :to="`/post/${post.slug}`" class="timeline-item">
            ...
            <div class="timeline-tag">
              <el-tag effect="plain" v-for="value in post.tags" type="info">
                {{ value.name }}
              </el-tag>
            </div>
          </RouterLink>
...
.scrollbar-flex-content button:hover {
 background: #fae3d9;
 box-shadow: 2px 2px #ffb6b9;
 border-color: #ffb6b9;
}

.scrollbar-flex-content button.active {
  box-shadow: 0 0 #000;
